...
def seed_currencies():
    ...
    currencies = [Currency(**c) for c in currencies_data]
    db.session.add_all(currencies)
    db.session.commit()
    return currencies
...
def seed_users(currencies):
    ...
    Currency.set_default_global_currency(currencies[0].id, users[2].id)
    db.session.add_all(users)
    db.session.commit()
    return users
...
def capitalize_nation(nation):
    return nation.upper() if len(nation) in (2, 3, 4) else nation.capitalize()

def seed_tanks():
    """Seeds the Tanks table with data from World of Tanks API."""

    console.print("📦 Fetching tank data from World of Tanks API...")
    try:
        response = requests.get(
            "https://api.worldoftanks.com/wot/encyclopedia/vehicles/",
        ...
        if tanks:
            db.session.add_all(tanks)
            db.session.commit()
            console.print(f"[green]✅ {len(tanks)} tanks successfully added to the database.[/green]")
...
def seed_payment_methods(users):
    methods = [
        PaymentMethod(user_id=users[0].id, cardholder_name='James May', card_number='89828281', expiry='09/21', cvv='926', type='debit', details='', default_method=False),
        PaymentMethod(user_id=users[0].id, cardholder_name='Richard Hammond', card_number='09837861', expiry='10/28', cvv='910', type='credit', details='This is Hammond\'s best card for purchases', default_method=True),
    ]
    db.session.add_all(methods)
    db.session.commit()

def seed_transactions(users, tanks):
    methods = PaymentMethod.query.filter_by(user_id=users[0].id).all()
    method_id = methods[0].id if methods else None

    transactions = [
        Transaction(user_id=users[0].id, payment_source='in_app_credit', transaction_purpose='purchase', amount=59.99, payment_method_id=method_id),
        Transaction(user_id=users[1].id, payment_source='in_app_gold', transaction_purpose='purchase', amount=tanks[1].price, payment_method_id=None),
        Transaction(user_id=users[0].id, payment_source='credit_card', transaction_purpose='purchase', amount=29.99, payment_method_id=None),
        Transaction(user_id=users[2].id, payment_source='debit_card', transaction_purpose='purchase', amount=102.99, payment_method_id=None),
    ]
    db.session.add_all(transactions)
    db.session.commit()

def seed_database():
    console.print("[bold cyan]\n🚀 Starting Tankify database seed...\n[/bold cyan]")
    start_time = perf_counter()

    with app.app_context():
        try:
            clear_database()
            console.print("✅ [green]Cleared database[/green]")

            currencies = seed_currencies()
            console.print("✅ [green]Seeded currencies[/green]")

            users = seed_users(currencies)
            console.print("✅ [green]Seeded users[/green]")
            ...
            seed_transactions(users, tanks)
            console.print("✅ [green]Seeded transactions[/green]")
            ...
            console.print(f"\n[bold magenta]✅ -------- Seeding Database Complete - Time {formatted_time}[/bold magenta]\n")

        except Exception as e:
            console.print(f"[bold red]❌ Error during seeding:[/bold red] {e}")
            db.session.rollback()
